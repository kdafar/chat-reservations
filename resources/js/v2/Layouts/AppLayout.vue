...
const navGates = {
    ...
    'insurance-followup':   { perm: 'view_any_insurance_claims' },
    'lab-orders':           { perm: 'view_any_lab_orders' },
    ...
    'payroll-runs':         { perm: 'view_any_payroll_runs' },
    'staff-loans':          { perm: 'view_any_staff_loans' },
    ...
    // Each module report has its own permission rather than borrowing the one on
    // the records behind it. Reading a stock list is not the same as reading what
    // the inventory is worth, and these gates must stay identical to the
    // abort(403) in each controller — the sidebar is a convenience, the
    // controller is the actual boundary.
    'stock-report':         { perm: 'view_stock_reports' },
    'payroll-report':       { perm: 'view_payroll_reports' },
    'insurance-report':     { perm: 'view_insurance_reports' },
    'purchasing-report':    { perm: 'view_purchasing_reports' },
    'bookings-report':      { perm: 'view_booking_reports' },
    'doctors-report':       { perm: 'view_doctor_reports' },
    'patients-report':      { perm: 'view_patient_reports' },
    'lab-report':           { perm: 'view_lab_reports' },
    'packages-report':      { perm: 'view_package_reports' },
    'discounts-report':     { perm: 'view_discount_reports' },
    'audit-report':         { perm: 'view_audit_reports' },
    clinics:                { perm: 'view_any_partner' },
    branches:               { perm: 'view_any_branch' },
    rooms:                  { perm: 'view_any_restaurant_table' },
    'payment-methods':      { perm: 'view_any_clinic_payment_methods' },
    'branch-closures':      { perm: 'view_any_branch_blackout' },
    gateways:               { perm: 'view_any_gateway_account' },
    roles:                  { perm: 'view_any_roles' },
...
const navSections = computed(() => ([
    ...
    },
    {
        ...
        items: [
            ...
            { id: 'insurance-followup',  icon: 'bell',           label: locale.value === 'ar' ? 'متابعة التحصيل' : 'Follow-up', href: '/admin/v2/insurance/follow-up', v2: true },
        ],
    },
    {
        id: 'lab', icon: 'beaker',
        ...
        items: [
            { id: 'lab-orders', icon: 'flask-conical', label: locale.value === 'ar' ? 'قائمة عمل المختبر' : 'Lab Worklist', href: '/admin/v2/lab-orders', v2: true },
    ...
    },
    {
        ...
        items: [
            ...
            { id: 'purchase-orders', icon: 'shopping-cart', label: locale.value === 'ar' ? 'أوامر الشراء' : 'Purchase Orders', href: '/admin/v2/purchase-orders', v2: true },
            { id: 'clinic-packages', icon: 'gift',         label: locale.value === 'ar' ? 'الباقات' : 'Packages', href: '/admin/v2/clinic-packages', v2: true },
            { id: 'gallery',         icon: 'image',        label: locale.value === 'ar' ? 'معرض النتائج' : 'Results Gallery', href: '/admin/v2/gallery', v2: true },
        ],
    },
    {
    ...
    },
    {
        ...
        label: locale.value === 'ar' ? 'الرواتب' : 'Payroll',
        items: [
            { id: 'payroll-runs',    icon: 'calendar-check', label: locale.value === 'ar' ? 'مسيّر الرواتب' : 'Payroll Runs',   href: '/admin/v2/payroll-runs', v2: true },
            { id: 'salary-profiles', icon: 'wallet',         label: locale.value === 'ar' ? 'هياكل الرواتب' : 'Salary Profiles', href: '/admin/v2/staff-compensation-profiles', v2: true },
            { id: 'staff-loans',     icon: 'hand-coins',     label: locale.value === 'ar' ? 'السلف والقروض' : 'Loans & Advances', href: '/admin/v2/staff-loans', v2: true },
            { id: 'leave-balances',  icon: 'calendar-clock', label: locale.value === 'ar' ? 'أرصدة الإجازات' : 'Leave Balances', href: '/admin/v2/leave-balances', v2: true },
            { id: 'settlements',     icon: 'log-out',        label: locale.value === 'ar' ? 'مكافأة نهاية الخدمة' : 'End of Service', href: '/admin/v2/staff-settlements', v2: true },
        ],
    },
    {
    ...
    },
    {
        ...
        items: [
            ...
            { id: 'stock-report',      icon: 'package',      label: locale.value === 'ar' ? 'تقرير المخزون' : 'Stock Report', href: '/admin/v2/reports/stock', v2: true },
            { id: 'payroll-report',    icon: 'users',        label: locale.value === 'ar' ? 'تقرير الرواتب' : 'Payroll & HR', href: '/admin/v2/reports/payroll', v2: true },
            { id: 'insurance-report',  icon: 'shield',       label: locale.value === 'ar' ? 'أداء التأمين' : 'Insurance Performance', href: '/admin/v2/reports/insurance', v2: true },
            { id: 'purchasing-report', icon: 'shopping-cart', label: locale.value === 'ar' ? 'المشتريات والموردين' : 'Purchasing & Vendors', href: '/admin/v2/reports/purchasing', v2: true },
            { id: 'bookings-report',   icon: 'calendar-check', label: locale.value === 'ar' ? 'تقرير المواعيد' : 'Appointments', href: '/admin/v2/reports/bookings', v2: true },
            { id: 'doctors-report',    icon: 'stethoscope',  label: locale.value === 'ar' ? 'إنتاجية الأطباء' : 'Doctor Productivity', href: '/admin/v2/reports/doctors', v2: true },
            { id: 'patients-report',   icon: 'user',         label: locale.value === 'ar' ? 'تقرير المرضى' : 'Patients & Clinical', href: '/admin/v2/reports/patients', v2: true },
            { id: 'lab-report',        icon: 'flask-conical', label: locale.value === 'ar' ? 'تقرير المختبر' : 'Laboratory', href: '/admin/v2/reports/lab', v2: true },
            { id: 'packages-report',   icon: 'gift',         label: locale.value === 'ar' ? 'تقرير الباقات' : 'Packages', href: '/admin/v2/reports/packages', v2: true },
            { id: 'discounts-report',  icon: 'percent',      label: locale.value === 'ar' ? 'الخصومات والعروض' : 'Discounts & Promotions', href: '/admin/v2/reports/discounts', v2: true },
            { id: 'audit-report',      icon: 'shield-check', label: locale.value === 'ar' ? 'التدقيق والامتثال' : 'Audit & Compliance', href: '/admin/v2/reports/audit', v2: true },
        ],
    },
    {
        ...
        items: [
            ...
            { id: 'rooms',      icon: 'door-open', label: locale.value === 'ar' ? 'الغرف' : 'Rooms', href: '/admin/v2/rooms', v2: true },
            { id: 'branch-closures', icon: 'calendar-off', label: locale.value === 'ar' ? 'أيام الإغلاق' : 'Closure Days', href: '/admin/v2/branch-closures', v2: true },
            { id: 'payment-methods', icon: 'wallet', label: locale.value === 'ar' ? 'طرق الدفع' : 'Payment Methods', href: '/admin/v2/payment-methods', v2: true },
...
const navDescriptions = {
    ...
    'insurance-followup':   { en: "Chase the money insurers still owe you — who owes what, how overdue it is, and a log of every follow-up.", ar: 'تابع تحصيل المستحقات من شركات التأمين — من عليه ماذا، ومدى تأخره، مع سجل لكل متابعة.' },
    // Lab
    'lab-orders':     { en: "The lab's live worklist — tests doctors ordered, sample collection, result entry and report release.", ar: 'قائمة عمل المختبر المباشرة — التحاليل التي طلبها الأطباء، سحب العينات، إدخال النتائج وإصدار التقارير.' },
    ...
    'purchase-orders':{ en: "Raise purchase orders to vendors, receive goods into stock (Dr Inventory / Cr Accounts Payable), and pay vendors — fully posted to accounting.", ar: 'إنشاء أوامر شراء للموردين، واستلام البضائع في المخزون (مدين المخزون / دائن الذمم الدائنة)، وسداد الموردين — مُرحَّل بالكامل إلى الحسابات.' },
    ...
    'gallery':        { en: "Before/after cases published on the public website. A case needs recorded patient consent before it can go live.", ar: 'حالات قبل وبعد تُنشر على الموقع العام. تحتاج كل حالة إلى موافقة موثّقة من المريضة قبل نشرها.' },
    // HR
    ...
    'stock-report':    { en: "What your inventory is worth per branch, what got consumed, which items are below their reorder point, and what is sitting unused. Also checks the on-hand value against the inventory account so a drift shows up instead of hiding.", ar: 'قيمة المخزون لكل فرع وما تم استهلاكه والأصناف تحت حد إعادة الطلب والمخزون الراكد، مع مطابقة القيمة مع حساب المخزون.' },
    'payroll-report':  { en: "What people cost the clinic and what the clinic owes them — payroll register, labour as a share of revenue, cost by branch and role, accrued leave liability, end-of-service provision, staff loans and attendance.", ar: 'تكلفة الموظفين والتزامات العيادة تجاههم — سجل الرواتب ونسبة العمالة للإيراد والتكلفة حسب الفرع والدور والتزام الإجازات ومخصص نهاية الخدمة والسلف والحضور.' },
    'insurance-report': { en: "How each insurer actually behaves: what share of billing they approve, how much of that they pay, how long they take, why claims get rejected, and how much is still outstanding by age.", ar: 'أداء كل شركة تأمين: نسبة الموافقة والتحصيل ومدة السداد وأسباب الرفض وأعمار المستحقات.' },
    'purchasing-report': { en: "Where the buying money goes — spend by vendor, where orders are stuck between raising and paying, payables aging, most-purchased items, and whether an item's price has been drifting between orders.", ar: 'أين تذهب أموال الشراء — الإنفاق حسب المورد وحالة الطلبات وأعمار الذمم الدائنة والأصناف الأكثر شراءً وتذبذب الأسعار.' },
    'bookings-report':  { en: "The demand side: no-show and cancellation rates, which booking channel brings patients who actually attend, how far ahead people book, busiest days and hours, and how long patients wait after checking in.", ar: 'جانب الطلب: نسب عدم الحضور والإلغاء وجودة قنوات الحجز ومدة الحجز المسبق وأكثر الأيام والساعات ازدحاماً ووقت الانتظار.' },
    'doctors-report':   { en: "Output measured against rostered time — revenue per hour, chair utilisation, average consultation length, follow-up conversion and commission per doctor. Shows who has capacity to spare.", ar: 'الإنتاجية مقابل ساعات العمل — الإيراد لكل ساعة ونسبة الاستغلال ومتوسط مدة الكشف ونسبة المتابعة والعمولة لكل طبيب.' },
    'patients-report':  { en: "Who your patients are and whether they come back — new vs returning, retention cohorts, lifetime value, demographics, treatment mix, and a recall list of patients who have lapsed.", ar: 'من هم مرضاك وهل يعودون — الجدد مقابل العائدين ومعدلات الاحتفاظ والقيمة الدائمة والتركيبة السكانية وقائمة المرضى المنقطعين.' },
    'lab-report':       { en: "Laboratory throughput and quality — turnaround time broken down by stage so you can see where time is lost, test volumes, abnormal result rates, and the pending backlog by age.", ar: 'أداء المختبر — زمن إنجاز الفحوصات لكل مرحلة وأحجام الفحوصات ونسب النتائج غير الطبيعية والطلبات المعلقة.' },
    'packages-report':  { en: "Package sales and whether they actually lift basket size — units sold, revenue, offer savings passed to patients, and average ticket on visits with a package versus without.", ar: 'مبيعات الباقات وأثرها على قيمة الزيارة — الكميات والإيرادات وقيمة العروض ومتوسط الفاتورة مع الباقة وبدونها.' },
    'audit-report':     { en: "Who did what, and when — voided payments, deleted records, discount overrides, after-hours edits, and the trail of who opened which patient file. Covers the whole estate; it cannot be narrowed to one branch because the activity log has no branch column.", ar: 'من فعل ماذا ومتى — المدفوعات الملغاة والسجلات المحذوفة وتجاوزات الخصم والتعديلات خارج ساعات العمل وسجل الاطلاع على ملفات المرضى.' },
    'discounts-report': { en: "What discounting costs you — discount as a share of billing, who gives the most, discount bands, coupon performance, active promotions, and the margin impact.", ar: 'تكلفة الخصومات — نسبة الخصم من الفوترة ومن يمنحها ونطاقات الخصم وأداء الكوبونات والعروض النشطة وأثرها على الهامش.' },
    // Platform
