...
@php
  ...
  $hasGoogle = true;   // set to false if you don't have the route
  $hasApple  = true;   // set to false if you don't have the route
  $hasTerms  = \Illuminate\Support\Facades\Route::has('terms.show') || \Illuminate\Support\Facades\Route::has('policy.show');
@endphp
    ...
    <div class="grid md:grid-cols-2 gap-6 items-stretch">
      ...
      <div class="hidden md:flex relative rounded-2xl overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 p-8 text-white">
        ...
        <div class="relative z-10 flex flex-col justify-end">
          <h2 class="text-2xl font-bold mb-2">{{ __('Join') }} {{ config('app.name') }}</h2>
          <p class="text-white/90">
            {{ __('Create your account to track orders, save addresses, and enjoy faster checkout.') }}
          </p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
        ...
        <div class="flex items-center gap-3 mb-6">
          ...
          <div class="text-gray-800 text-lg font-semibold">{{ __('Create account') }}</div>
        </div>
        ...
        {{-- Register form --}}
        <form method="POST" action="{{ route('register') }}" novalidate class="space-y-5" x-data="{ showPw:false, showPw2:false }">
          @csrf
          ...
          <div>
            ...
            <p class="mt-1 text-xs text-gray-500">{{ __('Use 8+ characters with a mix of letters and numbers.') }}</p>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <input id="password_confirmation" name="password_confirmation" :type="showPw2 ? 'text' : 'password'" autocomplete="new-password" required
                     class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 px-4 py-3 pr-12 text-gray-900 placeholder-gray-400"
                     placeholder="••••••••">
              <button type="button" @click="showPw2=!showPw2"
                      class="absolute {{ $rtl ? 'left-3' : 'right-3' }} top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                      aria-label="{{ __('Toggle password visibility') }}">
                <svg x-show="!showPw2" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 15.338 6.87 18 12 18c5.129 0 8.773-2.662 10.066-6a10.45 10.45 0 00-1.676-2.652M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                </svg>
                <svg x-show="showPw2" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <label class="flex items-start gap-2 text-sm">
              ...
              <span class="text-gray-700">
                {!! __('I agree to the :terms and :privacy', [
                      'terms' => \Illuminate\Support\Facades\Route::has('terms.show')
                        ? '<a class="text-indigo-600 hover:text-indigo-700 font-medium" href="'.route('terms.show').'" target="_blank">'.__('Terms').'</a>'
                        : __('Terms'),
                      'privacy' => \Illuminate\Support\Facades\Route::has('policy.show')
                        ? '<a class="text-indigo-600 hover:text-indigo-700 font-medium" href="'.route('policy.show').'" target="_blank">'.__('Privacy Policy').'</a>'
                        : __('Privacy Policy'),
                    ]) !!}
              </span>
            </label>
          ...
          <div class="relative text-center my-2">
            <span class="px-3 bg-white text-xs text-gray-400 relative z-10">{{ __('or sign up with') }}</span>
            <div class="absolute inset-x-0 top-1/2 -translate-y-1/2 h-px bg-gray-200"></div>
          </div>

          {{-- Social signups (optional) --}}
