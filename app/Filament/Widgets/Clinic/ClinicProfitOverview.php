...
class ClinicProfitOverview extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        // Keep filters for branch/doctor only
        ...
        $cacheKey = 'profit_overview_today:'.md5(json_encode([$today, $branchId, $doctorId, $userId]));

        $data = ClinicReportCache::remember($cacheKey, 60, function () use ($today, $branchId, $doctorId) {
        ...
        return [
            Stat::make('Visits (Today)', number_format((int) ($visitsAgg->visits_count ?? 0)))
                ->description('Computed visits (today)'),

            Stat::make('Fees (Today)', number_format((float) ($visitsAgg->fees_total ?? 0), 3))
                ->description('Sum fees_total (today)'),

            Stat::make('Items Cost (Today)', number_format((float) ($visitsAgg->items_cost_total ?? 0), 3))
                ->description('Sum items_cost_total (today)'),

            Stat::make('Profit (Today)', number_format((float) ($visitsAgg->profit_total ?? 0), 3))
                ->description('Sum profit_total (today)'),

            Stat::make('Doctor Cut (Today)', number_format((float) $doctorCut, 3))
                ->description('From compensation ledger (today)'),
        ];
    }

    protected function resolvedFilters(): array
    {
        // Keep existing parsing so branch/doctor filters still work,
